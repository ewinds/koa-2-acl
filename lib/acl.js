...
function config(config, response) {
  ...
  opt.lazyRules = options.lazyRules;
...
function authorize(ctx, next) {
  ...
  let role = ctx.state.user ? ctx.state.user.roles : [opt.defaultRole];

  if (opt.lazyRules && ctx.state.rules) {
    opt.rules = utils.validate(ctx.state.rules);
  }
   ...
   */
  if (!_.isArray(role)) {
    return utils.deny(ctx, 404, 'REQUIRED: Role should be provided as a array', null);
  }
  ...
  let groupPermissions = _.flattenDeep(_.values(_.pick(opt.rules, role)));
